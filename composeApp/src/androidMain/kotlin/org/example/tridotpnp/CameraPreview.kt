...
//                            val strokeW = (6f * ratio).coerceIn(1f, 8f)
        ...
        if (probabilityEditEnabled && probabilityMatrix != null) {
            ProbabilityEditOverlay(
                probabilityMatrix = probabilityMatrix,
                previewSize = previewSize,
                previewViewRef = previewViewRef,
                probabilityEditMode = probabilityEditMode,
                probabilityEditStep = probabilityEditStep,
                liveProbabilityMatrix = liveProbabilityMatrix,
                dragAnchorProbabilityDisplayCell = dragAnchorProbabilityDisplayCell,
                onLiveProbabilityMatrixChange = { liveProbabilityMatrix = it },
                onSelectedProbabilityDisplayCellChange = { selectedProbabilityDisplayCell = it },
                onLastEditedProbabilityDisplayCellChange = { lastEditedProbabilityDisplayCell = it },
                onDragAnchorProbabilityDisplayCellChange = { dragAnchorProbabilityDisplayCell = it },
                onProbabilityCellSelected = onProbabilityCellSelected,
                onProbabilityMatrixChange = onProbabilityMatrixChange,
                haptic = haptic
            )
        }
